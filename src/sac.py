...
class SAC_countinuous():
	def __init__(self, **kwargs):
		...
		self.replay_buffer = ReplayBuffer(self.state_dim, self.action_dim, max_size=self.replay_buffer_size, device=self.device)
	...
	def train(self,):
		...
		a_loss = (self.alpha * log_pi_a - Q).mean() # SAC中的\pi(·|s)的entropy就是阿紫这里实现的
